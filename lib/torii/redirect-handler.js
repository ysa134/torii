...
if ((new Function('/*@cc_on return document.documentMode===10@*/'))()) {
  // For < IE11
  postMessageToParent = function postMessageToParent(data, host) {
    window.opener.postMessageWithFix(data, host);
  };
} else {
  postMessageToParent = function postMessageToParent(data, host) {
    window.opener.postMessage(data, host);
  };
}
  ...
  run: function(){
    ...
    return new Ember.RSVP.Promise(function(resolve, reject){
      if (window.opener && window.opener.name === 'torii-opener') {
        ...
        postMessageToParent(data, window.location.protocol+'//'+window.location.host);
